	...
	public City calcGoalCity(Bag citylist) { // returns the best city
		...
		for (Object city : citylist) {
			...
			double cityDesirability = dangerCare() * c.getViolence() 
					...
					+ c.getEconomy() * 
					(Parameters.ECON_CARE + random.nextDouble()*(1-Parameters.ECON_CARE)) //adds randomness
					+ c.getScaledPopulation() *
					(Parameters.POP_CARE + random.nextDouble()*(1-Parameters.POP_CARE)); //adds randomness
